...
// ═══════════════════════════════════════════════════════════════════════════
// ─── HARGA LIST (getPricesV3 per negara + service) ─────────────────────────
// Return semua provider untuk service+country tertentu, diurutkan harga murah-mahal.
// ═══════════════════════════════════════════════════════════════════════════

/**
 * Ambil list harga dari getPricesV3 untuk kombinasi service+country.
 * Setiap entry = 1 provider dengan harga IDR sudah di-markup.
 * @returns { success, data: [{ id, label, price, price_format, stock, provider_id, price_usd, _sub }] }
 */
async function getPricesListV3SmsBower(serviceCode, countryId) {
  const markup   = await getMarkupAsync();
  const usdToIdr = await getUsdToIdrAsync();
  const CACHE_KEY = `sb:priceslist3:${serviceCode}:${countryId}:m${markup}:r${usdToIdr}`;
  const cached = cacheGet(CACHE_KEY);
  if (cached) return cached;

  const res = await fetchSmsBower({ action: 'getPricesV3', service: serviceCode, country: countryId });
  if (!res.success || !res._json) return { success: false, data: [] };
  ...
  const providers = countryData[serviceCode];
  if (!providers || typeof providers !== 'object') return { success: false, data: [] };
  ...
  for (const provEntry of Object.values(providers)) {
    ...
    const price = Math.ceil(usdP * usdToIdr * markup);
    data.push({
      id: pid,
      provider_id: pid,
      price,
      ...
      price_format: `Rp${price.toLocaleString('id-ID')}`,
      stock: cnt,
      label: `Rp${price.toLocaleString('id-ID')} · stok: ${cnt.toLocaleString()}`,
    });
  }
...
export async function getPricesListV3(serviceCode, countryId) {
  return getPricesListV3SmsBower(serviceCode, countryId);
}
...
async function getOrderStatusSmsBower(orderId) {
  ...
  // HTTP error — cek apakah raw mengandung info cancel
  if (!res.success) {
    const rawLow = raw.toLowerCase();
    if (rawLow.includes('no_activation') || rawLow.includes('expired') || rawLow.includes('not found')) {
      console.log(`[SmsBower getStatus] order=${orderId} → HTTP error but looks canceled: ${raw}`);
      return { success: true, data: { status: 'canceled', otp_code: null } };
    }
    // Unknown error → jangan stuck, anggap waiting sementara
  ...
  // NO_ACTIVATION = aktivasi sudah tidak ada di provider (expired/canceled di sisi mereka)
  if (raw === 'NO_ACTIVATION' || raw === 'BAD_STATUS') {
    console.log(`[SmsBower getStatus] order=${orderId} → ${raw} = treat as canceled`);
    return { success: true, data: { status: 'canceled', otp_code: null } };
  }
  ...
  // Fallback: response tidak dikenal → anggap canceled (tidak mau stuck di 'waiting')
  console.warn(`[SmsBower getStatus] order=${orderId} unrecognized response: "${raw}" → treating as canceled`);
  return { success: true, data: { status: 'canceled', otp_code: null } };
}
