...
async function getCountriesForServiceSmsBower(serviceCode) {
  ...
  // V2: { "countryCode": { "serviceCode": { "price": count, ... } } }
  const entries = [];
  for (const [countryId, services] of Object.entries(res._json)) {
    ...
    for (const [priceStr, count] of Object.entries(priceTiers)) {
      ...
      totalStock += cnt;
      validTiers.push({ usdP, cnt });
    }
    if (totalStock <= 0 || validTiers.length === 0) continue;

    const minSignificantStock = Math.max(Math.ceil(totalStock * 0.01), 500);
    const significantTiers = validTiers.filter(t => t.cnt >= minSignificantStock);
    const tiersToUse = significantTiers.length > 0 ? significantTiers : validTiers;
    const representativeUsd = Math.min(...tiersToUse.map(t => t.usdP));
    const price = Math.ceil(representativeUsd * usdToIdr * markup);
    entries.push({
      id: String(countryId),
...
async function getServicesForCountrySmsBower(countryId) {
  // Selalu gunakan async getter agar DB di-load saat cache kosong (cold-start)
  const markup   = await getMarkupAsync();
  const usdToIdr = await getUsdToIdrAsync();
  // Cache key menyertakan markup + rate agar cache otomatis invalid saat rate berubah
  ...
  // V2 response: { "countryCode": { "serviceCode": { "0.008": 1000, "0.174": 163804 } } }
  ...
  for (const [code, priceTiers] of Object.entries(countryData)) {
    ...
    // ── PRICE STRATEGY: Min harga dari tier yang punya stok signifikan ──
    // SMS Bower assign nomor dari tier TERMURAH yang masih ada stok.
    // Tapi tier dengan stok sangat kecil (<1% total) adalah flash/reserved stock
    // yang habis secepat kilat → SMS Bower akan pakai tier berikutnya yang lebih mahal.
    // Solusi: skip tier micro-stock, ambil minimum dari tier yang reliably tersedia.
    let totalStock  = 0;
    let weightedSum = 0; // kept for logging
    const validTiers = [];
    ...
    for (const [priceStr, count] of Object.entries(priceTiers)) {
      ...
      if (cnt <= 0 || usdP <= 0) continue;
      totalStock  += cnt;
      weightedSum += usdP * cnt;
      validTiers.push({ usdP, cnt });
    }
    if (totalStock <= 0 || validTiers.length === 0) continue;

    // Filter: hanya tier dengan stok ≥ 1% dari total (atau minimal 500 nomor)
    const minSignificantStock = Math.max(Math.ceil(totalStock * 0.01), 500);
    const significantTiers = validTiers.filter(t => t.cnt >= minSignificantStock);
    // Fallback: kalau semua tier kecil (layanan langka), pakai semua tier
    const tiersToUse = significantTiers.length > 0 ? significantTiers : validTiers;

    // Ambil harga TERMURAH dari tier yang signifikan = estimasi harga SMS Bower
    const representativeUsd = Math.min(...tiersToUse.map(t => t.usdP));
    const price = Math.ceil(representativeUsd * usdToIdr * markup);

    const minPrice = Math.min(...validTiers.map(t => t.usdP));
    const maxPrice = Math.max(...validTiers.map(t => t.usdP));
    console.log(`[price] ${code}@${countryId}: tiers=${validTiers.length} significant=${tiersToUse.length} min=$${minPrice} repr=$${representativeUsd} max=$${maxPrice} → Rp${price} (rate=${usdToIdr}, markup=${markup}x)`);
