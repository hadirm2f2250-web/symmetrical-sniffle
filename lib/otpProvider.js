...
async function getCountriesForServiceSmsBower(serviceCode) {
  ...
  for (const [countryId, services] of Object.entries(res._json)) {
    ...
    if (totalStock <= 0 || weightedSum === 0) continue;

    const weightedAvgUsd = weightedSum / totalStock;
    const price = Math.ceil(weightedAvgUsd * usdToIdr * markup);
    entries.push({
      id: String(countryId),
...
async function getServicesForCountrySmsBower(countryId) {
  ...
  for (const [code, priceTiers] of Object.entries(countryData)) {
    ...
    // ── PRICE STRATEGY: Weighted average berdasarkan stok ──
    // Alasan: SMS Bower assign nomor dari pool sesuai distribusi stok.
    // Tier dengan stok terbanyak = tier yang paling mungkin di-assign.
    // Min price tier bisa sangat murah tapi stoknya sedikit → user akan dicharge tier lebih mahal.
    let totalStock  = 0;
    let weightedSum = 0; // sum(price × stock)
    const validTiers = [];
    ...
    // Harga rata-rata tertimbang stok → mendekati harga yang sebenarnya akan di-charge
    const weightedAvgUsd = weightedSum / totalStock;
    const price = Math.ceil(weightedAvgUsd * usdToIdr * markup);

    const minPrice = Math.min(...validTiers.map(t => t.usdP));
    const maxPrice = Math.max(...validTiers.map(t => t.usdP));
    console.log(`[price] ${code}@${countryId}: tiers=${validTiers.length} min=$${minPrice} max=$${maxPrice} wavg=$${weightedAvgUsd.toFixed(4)} → Rp${price} (rate=${usdToIdr}, markup=${markup}x)`);
