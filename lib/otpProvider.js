...
// Server 4: RuangOTP — fetch all services, then get countries for each, deduplicate
async function getCountriesServer4() {
  const svcRes = await fetchRuangOTP('/v2/services');
  if (!svcRes.success || !svcRes.data?.length) return { success: false, data: [] };
  ...
  results.forEach(r => {
    ...
    r.data.forEach(c => {
      if (!seen.has(c.name)) {
        seen.set(c.name, { id: String(c.number_id), name: c.name, iso_code: c.iso_code });
      }
    });
  });
...
// Server 4: get services available for a specific country (with pricelist/provider embedded)
export async function getServicesForCountryServer4(countryName) {
  ...
  results.forEach(({ service, result }) => {
    ...
    const country = result.data.find(c => c.name.toLowerCase() === countryName.toLowerCase());
    if (!country) return;

    const pricelist = (country.pricelist || []).filter(p => p.available);
    if (!pricelist.length) return;

    // Pick cheapest provider
    ...
    available.push({
      ...
      number_id: String(country.number_id),
      price: best.price,
...
export async function getCountries(server = 'server3') {
  if (server === 'server4') return getCountriesServer4();
